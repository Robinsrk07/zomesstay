...
const HostLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(dataHost.host.id)
       let dataProperty = null;
       
      if(dataHost.host.id){
      
        const response = await inventoryService.getPropertyByID(dataHost.host.id);
         dataProperty = response?.data?.data
      } 

       console.log(dataProperty)
     
      sessionStorage.setItem("authToken",dataHost.token);
       dispatch(setLogin({
        email: dataHost.host.email,
        role: dataHost.host.role,
        first_name: dataHost.host.firstName,
        last_name: dataHost.host.lastName,
        profileImage: dataHost.host.profileImage,
        id: dataHost.host.id
       }));
       dispatch(setHostProperty({
        id: dataProperty.id,
        name: dataProperty.title,
       }));
